...
public class CacheChannel extends ReceiverAdapter implements CacheExpiredListener {
	...
	@SuppressWarnings("rawtypes")
	public void evictPrefix(String region, String keyPrefix) {
		List keys = keys(region);
		if(keys != null){
			for(Object o : keys){
				if(o != null){
					String str = o.toString();
					if(StringUtils.startsWith(str, keyPrefix)){
						evict(region, str);
					}
				}
			}
	...
	@SuppressWarnings("rawtypes")
	public void evictPrefix(String region, List<String> keyPrefix) {
		List keys = keys(region);
		if(keys != null){
			for(Object o : keys){
				if(o != null){
					String str = o.toString();
					for(String tempKey : keyPrefix){
						if(StringUtils.startsWith(str, tempKey)){
							evict(region, str);
						}
					}
				}
